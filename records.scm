...
(define-record cfg
  ...
  ;; This is a list containing either symbols or CFG-PRECEDENCE.  These
  ;; represent the terminal symbols in the grammar.  Precedence records
  ;; appearing earlier in the list have higher precedence than those appearing
  ;; later.  Symbols represent terminal symbols with no defined precedence.
  terminals
...
(define-record cfg-rule
  ...
  ;; then the precedence for the rule will be the precedence of the last
  ;; terminal symbol in the rule (if any).
  precedence
...
(define-record LR-program
  ...
  ;; This is the list of terminal symbols.  Put another way, the elements in
  ;; this list in addition to the symbol in EOI are the only symbols that are
  ...
  ;; This is the symbol that is shifted when errors are encountered.  If
  ;; defined, in will appear in RULES and STATES but should never be returned by
  ;; the lexer.  If not defined, this entry will be #f.
  error
  ...
  ;; the PDA.  The start state is at index 0.
  states)

;;; The LR-program:rules field will contain a vector of these records.
(define-record LR-rule

  ;; This is the non-terminal for the reduction.  It is used to index into the
  ;; goto-table after the reduction takes place.
  left-side
...
(define-record LR-state
  ...
  ;; taken if no other rule matches.  The default action is always a reduce.  If
  ;; the action is 'shift, the STATE-NUM gives the index of the state to shift
  ;; into.  If the action is 'reduce, RULE-NUM gives the index of the rule to
  ;; reduce by.  The 'accept action terminates the PDA.
  shift-reduce-table
  ...
  ;; index.  Note that if the "dot" is at the end of the rule, the index will
  ;; not be valid.
  items)
