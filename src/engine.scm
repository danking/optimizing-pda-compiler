...
(define (parse/pda-macro form rename compare)
  (if (not (member (length form) '(5 6)))
      (error "Wrong number of arguments to parse/pda"))
  (let ((get-token (second form))
	(drop-token (third form))
	(token-case (fourth form))
	(parse-error (if (= (length form) 6)
			 (fifth form)
			 `(,(rename 'lambda) (x y)
	   ...
	   (case (caar form)
	     ...
	     ((ERROR)
	      (cond ((not (= (length (car form)) 2))
		    ...
		    (else
		     (loop no-shift rules states start-state (cadar form)
				(cdr form)))))
	     ((NO-SHIFT)
		  ...
		  (,%reduce
		    (,%lambda (value-s goto-s error-s stream dist rule)
		      (,%let* ((non-terminal (,%vector-ref rule 0))
			       ...
			       (goto-s (,%drop goto-s arity))
			       (error-s (,%drop error-s arity)))
			(,%let loop ((i 0) (value-s value-s) (call-values '()))
			  (,%if (,%< i arity)
				(loop (,%+ i 1) (,%cdr value-s)
				      (,%cons (,%car value-s) call-values))
				((,%car goto-s)
				 (,%cons (,%apply action call-values)
					 value-s)
				 goto-s error-s stream dist
				 non-terminal))))))
		  (,%pda-error
